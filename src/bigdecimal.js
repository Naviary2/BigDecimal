...
const bimath_js_1 = require("./bimath.js");
// Config ===========================================================
/**
...
// Constants ========================================================
const LOG10_OF_2 = Math.log10(2); // ≈ 0.30103
 ...
 */
function FromNumber(num, precision = DEFAULT_WORKING_PRECISION) {
    ...
    if (num === 0)
        return { bigint: ZERO, divex: precision };
 ...
 */
function FromBigInt(num, precision = DEFAULT_WORKING_PRECISION) {
    if (precision < 0 || precision > MAX_DIVEX)
        throw new Error(`Precision must be between 0 and ${MAX_DIVEX}. Received: ${precision}`);
    return {
...
function howManyBitsForDigitsOfPrecision(precision) {
    if (precision === 0)
        return 0; // No bits needed for zero precision.
    // Use bigints so that in-between values don't become Infinity.
    const powerOfTen = TEN ** BigInt(precision); // 3 ==> 1000n
    // 2^x = powerOfTen. Solve for x
    return bimath_js_1.default.log2(powerOfTen) + 1; // +1 to round up
}
 ...
 */
function multiply_fixed(bd1, bd2) {
 ...
 */
function multiply_floating(bd1, bd2, mantissaBits) {
 ...
 */
function divide_fixed(bd1, bd2, workingPrecision = DEFAULT_WORKING_PRECISION) {
    if (bd2.bigint === ZERO)
 ...
 */
function divide_floating(bd1, bd2, mantissaBits = DEFAULT_MANTISSA_PRECISION_BITS) {
    if (bd2.bigint === ZERO)
        throw new Error('Division by zero is not allowed.');
    if (bd1.bigint === ZERO)
        return { bigint: ZERO, divex: mantissaBits }; // Or any divex, normalize will handle it.
    // 1. Calculate bit length of the absolute values for a magnitude comparison.
    const len1 = bimath_js_1.default.bitLength_bisection(bimath_js_1.default.abs(bd1.bigint));
    const len2 = bimath_js_1.default.bitLength_bisection(bimath_js_1.default.abs(bd2.bigint));
 ...
 */
function powerInt(base, exp) {
    if (!Number.isInteger(exp))
        throw new Error('Exponent must be an integer. Received: ' + exp);
    ...
    while (exp > 0) {
        ...
        if (exp % 2 === 1)
            res = multiply_floating(res, currentPower);
 ...
 */
function pow(base, exponent, mantissaBits = DEFAULT_MANTISSA_PRECISION_BITS) {
    // 1. Handle edge cases
    ...
    if (base.bigint === ZERO) {
        if (exponent > 0)
            return { bigint: ZERO, divex: mantissaBits }; // 0^positive = 0
        if (exponent < 0)
 ...
 */
function sqrt(bd, mantissaBits = DEFAULT_MANTISSA_PRECISION_BITS) {
    // 1. Validate input
    if (bd.bigint < ZERO)
        throw new Error('Cannot calculate the square root of a negative number.');
    if (bd.bigint === ZERO)
        return { bigint: ZERO, divex: bd.divex };
    ...
    const bitLength = bimath_js_1.default.bitLength_bisection(bd.bigint) - bd.divex; // Subtract the decimal bits
    let x_k = {
        bigint: ONE,
 ...
 * The precision of the result is determined by the `mantissaBits` parameter.
 */
function hypot(bd1, bd2, mantissaBits = DEFAULT_MANTISSA_PRECISION_BITS) {
    // 1. Square the inputs
    const bd1_squared = multiply_fixed(bd1, bd1);
    const bd2_squared = multiply_fixed(bd2, bd2);
    // 2. Add the squares together.
    const sum_of_squares = add(bd1_squared, bd2_squared);
...
function abs(bd) {
    return {
        bigint: bimath_js_1.default.abs(bd.bigint),
        divex: bd.divex,
    };
}
 ...
 */
function hasDefaultPrecision(bd) {
...
function ln(bd) {
    ...
    const logOfMantissa = bimath_js_1.default.ln(bd.bigint);
 ...
 */
function exp(bd, mantissaBits = DEFAULT_MANTISSA_PRECISION_BITS) {
 ...
 */
function normalize(bd, precisionBits = DEFAULT_MANTISSA_PRECISION_BITS) {
    // We work with the absolute value for bit length calculation.
    const mantissa = bimath_js_1.default.abs(bd.bigint);
    // Use the fast, mathematical bitLength function.
    const currentBitLength = bimath_js_1.default.bitLength_bisection(mantissa);
 ...
 */
function toBigInt(bd) {
    // Negative divex means it's a large integer. Scale up.
    if (bd.divex < 0)
        return bd.bigint << BigInt(-bd.divex);
...
function toNumber(bd) {
    if (bd.divex >= 0) {
        if (bd.divex > MAX_DIVEX_BEFORE_INFINITY)
            return 0; // Exponent is so high that the resulting number cast to that power of two will be close to zero.
        const mantissaAsNumber = Number(bd.bigint);
        if (!isFinite(mantissaAsNumber))
            return mantissaAsNumber; // Already Infinity or -Infinity
        ...
        if (!isFinite(mantissaAsNumber))
            return mantissaAsNumber; // Already Infinity or -Infinity
        if (exp > MAX_DIVEX_BEFORE_INFINITY)
            return mantissaAsNumber >= 0 ? Infinity : -Infinity; // Exponent is so high that the resulting number cast to that power of two will be infinite.
 ...
 */
function toExactString(bd) {
    if (bd.bigint === ZERO)
        return '0';
    if (bd.divex < 0)
        return toBigInt(bd).toString(); // Negative divex: It's a large integer.
    if (bd.divex === 0)
        return bd.bigint.toString();
 ...
 */
function toApproximateString(bd) {
    ...
    const absStr = bimath_js_1.default.abs(roundedScaledInt).toString();
    let integerPart;
    let fractionalPart;
    ...
    if (trimmedFractionalPart.length === 0)
        ...
        return sign + integerPart + '.' + trimmedFractionalPart;
}
/**
 ...
 */
function toDebugBinaryString(bd) {
    return bimath_js_1.default.toDebugBinaryString(bd.bigint);
}
/**
 ...
 */
function getEffectiveDecimalPlaces(bd) {
